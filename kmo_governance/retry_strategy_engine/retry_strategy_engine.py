...
class RetryEngine:
    ...
    def compute_delay(self, attempt: int, config: RetryConfig) -> float:
        ...
        # Resolve strategy: enum value first, custom_strategies second
        strategy_value = config.strategy.value
        with self._lock:
            if config.strategy in _BUILTIN_STRATEGIES:
                delay_fn = _BUILTIN_STRATEGIES[config.strategy]
            elif strategy_value in self._custom_strategies:
                delay_fn = self._custom_strategies[strategy_value]
            else:
                raise ValueError(
                    f"unknown strategy: {strategy_value}"
                )
